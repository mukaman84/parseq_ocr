...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool) -> Optional[STEP_OUTPUT]:
        ...
        # print("start _eval_step", str(validation))
        ...
        if validation:
            try:
                ...
                # print("val loss", loss, loss_numel)
                val = str(loss.detach().cpu().numpy())
                if val=='nan':
                    print("nan occur")
                else:
                    pass
                ...
                print("err val loss", loss, loss_numel)
                print(e)
        else:
    ...
    def validation_epoch_end(self, outputs: EPOCH_OUTPUT) -> None:
        ...
        print("val acc : {} loss : {}".format(str(acc), str(loss).split("(")[-1].split(",")[0]))
